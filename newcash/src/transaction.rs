...
use gtk::{
    ...
    GtkListStoreExt, GtkMenuItemExt, GtkWindowExt, Inhibit, Label, ListStore, Menu, MenuItem,
    MenuShellExt, ResponseType, TreeModel, TreeModelExt, TreePath, TreeSelectionExt, TreeView,
    TreeViewColumn, TreeViewColumnExt, TreeViewExt, WidgetExt, Window, WindowType,
};
use queries::{
...
fn new_split(transaction_register: &TransactionRegister, globals: &Globals) {
    ...
    let new_split_guid = prepare_statement!(NEW_UUID_SQL, globals)
        .query_row(params![], get_result!(string))
        .unwrap();
    prepare_statement!(NEW_SPLIT_SQL, globals)
        .execute(params![
            new_split_guid,
...
fn duplicate_split(transaction_register: &TransactionRegister, globals: &Globals) {
    if let Some((model, iter)) = get_selection_info(&transaction_register.core, globals) {
        let view = &transaction_register.core.view;
        let source_split_guid: String =
            model.get_value(&iter, STORE_SPLIT_GUID).get().unwrap().unwrap();
        let destination_split_guid = prepare_statement!(NEW_UUID_SQL, globals)
            .query_row(params![], get_result!(string))
            .unwrap();
        // Create the new split
        prepare_statement!(DUPLICATE_SPLIT_SQL, globals)
            .execute(params![destination_split_guid, source_split_guid])
            .unwrap();
        refresh_transaction_registers(&TransactionChanged, &transaction_register.guid, globals);
        select_row_by_guid(
            view,
...
fn delete_split(transaction_register: &TransactionRegister, globals: &Globals) {
    if let Some((model, iter)) = get_selection_info(&transaction_register.core, globals) {
        ...
        let view = &transaction_register.core.view;
        let split_guid: String = model.get_value(&iter, STORE_SPLIT_GUID).get().unwrap().unwrap();
        prepare_statement!(DELETE_SPLIT_SQL, globals).execute(params![split_guid]).unwrap();
        refresh_transaction_registers(&TransactionChanged, &transaction_register.guid, globals);
...
fn reverse_sign(transaction_register: &TransactionRegister, globals: &Globals) {
    if let Some((model, iter)) = get_selection_info(&transaction_register.core, globals) {
        let view = &transaction_register.core.view;
        let split_guid: String = model.get_value(&iter, STORE_SPLIT_GUID).get().unwrap().unwrap();
        prepare_statement!(REVERSE_SIGN_SQL, globals).execute(params![split_guid]).unwrap();
...
fn copy_split_account_path_to_clipboard(
    transaction_register: &TransactionRegister, globals: &Globals,
) {
    if let Some((model, iter)) = get_selection_info(&transaction_register.core, globals) {
        let split_account_guid: String =
            model.get_value(&iter, STORE_ACCOUNT_GUID).get().unwrap().unwrap();
        let full_split_account_path = guid_to_path(
            prepare_statement!(GUID_TO_PATH_SQL, globals),
...
fn copy_split_account(transaction_register: &TransactionRegister, globals: &Globals) {
    if let Some((model, iter)) = get_selection_info(&transaction_register.core, globals) {
        let account_guid = model.get_value(&iter, STORE_ACCOUNT_GUID).get().unwrap().unwrap();
...
fn paste_split_account(transaction_register: &TransactionRegister, globals: &Globals) {
    ...
    if let Some(account_copy_buffer) = (*globals.account_copy_buffer.borrow()).as_ref() {
        if let Some((model, iter)) = get_selection_info(&transaction_register.core, &globals) {
            let split_guid: String =
                model.get_value(&iter, STORE_SPLIT_GUID).get().unwrap().unwrap();
            prepare_statement!(PASTE_ACCOUNT_GUID_SQL, globals)
                .execute(&[account_copy_buffer, &split_guid])
                .unwrap();
            refresh_transaction_registers(&SplitEdited, &split_guid, globals);
            refresh_account_registers(None, Some(&transaction_register.guid), globals);
        }
    } else {
        display_message_dialog("Cannot paste a split account without first doing a copy", globals);
    }
}

fn display_split_account_register(
    transaction_register: &TransactionRegister, globals: &Rc<Globals>,
) {
    if let Some((model, iter)) = get_selection_info(&transaction_register.core, &globals) {
        let account_guid: String =
            model.get_value(&iter, STORE_ACCOUNT_GUID).get().unwrap().unwrap();
        let marketable = inherited_p(
            prepare_statement!(INHERITED_P_SQL, globals),
            &account_guid,
...
fn flag_bit_toggled(
    renderer: &CellRendererToggle, path: &TreePath, transaction_register: &TransactionRegister,
    sql: &str, store_column_index: i32, globals: &Globals,
) {
    let store = &transaction_register.store;
    let split_guid: String = get_string_column_via_path(store, path, STORE_SPLIT_GUID);

    // Update the database
    prepare_statement!(sql, globals).execute(params![split_guid]).unwrap();
...
fn update_balancing_split(
    transaction_register: &TransactionRegister, split_guid: &str, globals: &Globals,
) {
    ...
    let nrows = prepare_statement!(SPLIT_COUNT_SQL, globals)
        .query_row(params![transaction_register.guid], get_result!(i32))
        .unwrap();
    ...
    if nrows == 2 {
        ...
        if let Some(stmt) = maybe_stmt {
            stmt.execute(params![transaction_register.guid, split_guid]).unwrap();
        }
    }
}
...
fn value_edited(
    path: &TreePath, new_value_expression: &str, transaction_register: &TransactionRegister,
    globals: &Globals,
) {
    let view = &transaction_register.core.view;
    ...
    let split_account_guid: String =
        model.get_value(&iter, STORE_ACCOUNT_GUID).get().unwrap().unwrap();
    let split_shares_p = inherited_p(
        prepare_statement!(INHERITED_P_SQL, globals),
        &split_account_guid,
        ACCOUNT_FLAG_DESCENDENTS_ARE_MARKETABLE,
    );
        ...
        evaluate_expression(sanitize(new_value_expression).as_str(), globals)
    {
        ...
        display_message_dialog(
            "Invalid expression when editing the value field of a split",
            globals,
        );
        return;
    };
    ...
    if split_shares_p {
        ...
        if money_market_p(&split_guid, globals) {
            ...
            let full_account_name: String =
                model.get_value(&iter, STORE_FULL_ACCOUNT_NAME).get().unwrap().unwrap();
            display_message_dialog(
                format!(
...
edit this split.",
        ...
        prepare_statement!(UPDATE_VALUE_SQL, globals)
            .execute(params![new_value, split_guid])
            .unwrap();
    }
    update_balancing_split(&transaction_register, &split_guid, globals);
...
fn price_edited(
    path: &TreePath, new_price_expression: &str, transaction_register: &TransactionRegister,
    globals: &Globals,
) {
    let view = &transaction_register.core.view;
    let model = view.get_model().unwrap();
        ...
        evaluate_expression(sanitize(new_price_expression).as_str(), globals)
    {
        ...
        display_message_dialog(
            "Invalid expression when editing the price field of a split",
            globals,
        );
        return;
    };
    ...
    let (quantity_nullzero_ness, value_nullzero_ness) =
        prepare_statement!(PRICE_EDITED_NULL_CHECK_SQL, globals)
            .query_row(params![split_guid], get_result!(bool_bool))
            .unwrap();
    let case_1_stmt =
        prepare_statement!("update splits set value=?1, quantity = 1.0 where guid = ?2", globals);
    let case_2a_stmt =
        prepare_statement!("update splits set quantity=value/(?1) where guid = ?2", globals);
    let case_2b_stmt =
        prepare_statement!("update splits set value=quantity*(?1) where guid = ?2", globals);
    let case_3a_stmt =
        prepare_statement!("update splits set value=quantity*(?1) where guid = ?2", globals);
    let case_3b_stmt = prepare_statement!(
        "update splits set quantity=ifnull(value/(?1),0) where \
         guid = ?2",
        globals
    );
    let stmt = if quantity_nullzero_ness && value_nullzero_ness {
        ...
        let dialog = Dialog::new_with_buttons(
            Some("Choose field to re-compute"),
            Some(&transaction_register.core.window),
            DialogFlags::MODAL,
            &[("Value", VALUE), ("Shares", QUANTITY)],
        );
        let content_area = dialog.get_content_area();
        content_area.add(&Label::new(Some(
            "Price equals Value / Shares.\nUnlike Shares and \
...
fn quantity_edited(
    path: &TreePath, new_quantity_expression: &str, transaction_register: &TransactionRegister,
    globals: &Globals,
) {
    let view = &transaction_register.core.view;
    let model = view.get_model().unwrap();
        ...
        evaluate_expression(sanitize(new_quantity_expression).as_str(), globals)
    {
        ...
        display_message_dialog(
            "Invalid expression when editing the quantity field of a split",
            globals,
        );
        return;
    };
    if money_market_p(&split_guid, globals) {
        ...
        prepare_statement!(UPDATE_QUANTITY_SQL, globals)
            .execute(params![new_quantity, split_guid])
            .unwrap();
    };
    update_balancing_split(transaction_register, &split_guid, globals);
...
pub fn refresh_transaction_registers(reason: &WhatChanged, guid: &str, globals: &Globals) {
    fn refresh_transaction_register(
        transaction_register: &TransactionRegister, reason: &WhatChanged, guid: &str,
        globals: &Globals,
    ) {
        let view = &transaction_register.core.view;
        ...
        loop {
            let stored_guid: String = model.get_value(&iter, column_index).get().unwrap().unwrap();
            if stored_guid == guid {
...
fn memo_edited(
    path: &TreePath, new_memo: &str, transaction_register: &TransactionRegister, globals: &Globals,
) {
    ...
    prepare_statement!(UPDATE_MEMO_SQL, globals).execute(params![temp, split_guid]).unwrap();
...
fn populate_transaction_register_store(
    transaction_register: &TransactionRegister, globals: &Globals,
) {
...
pub fn create_transaction_register(
    transaction_guid: String, description: String, date: &str,
    ...
    if globals.transaction_registers.borrow().contains_key(&transaction_guid) {
        display_message_dialog(
            &format!("A transaction register already exists for {}", description),
            &globals,
        );
    } else {
        let transaction_register = Rc::new(TransactionRegister {
            account_register: account_register.clone(),
            core: RegisterCore {
                view: TreeView::new(),
            ...
            store: create_transaction_store(account_register.shares_p),
        });

        // Record the the descriptor in the transaction_registers hashtable
        globals
            .transaction_registers
            .borrow_mut()
            .insert(transaction_register.guid.clone(), transaction_register.clone());
        ...
        // Memo
        {
            ...
            renderer.connect_edited(move |_, path, new_memo| {
                memo_edited(&path, new_memo, &(*closure_transaction_register), &closure_globals)
            });
        ...
        if transaction_register.account_register.shares_p {
            ...
            // Quantity
            {
                ...
                renderer.connect_edited(move |_, path, new_quantity| {
                    quantity_edited(&path, &new_quantity, &closure_transaction_register, &globals)
                });
                renderer.set_property_editable(true);
                let column: TreeViewColumn =
            ...
            // Price
            {
                ...
                renderer.connect_edited(move |_, path, new_price| {
                    price_edited(&path, &new_price, &(*closure_transaction_register), &globals)
                });
                renderer.set_property_editable(true);
                let column: TreeViewColumn =
            ...
            // Value
            {
                ...
                let closure_transaction_register = transaction_register.clone();
                renderer.connect_edited(move |_, path, new_factor| {
                    value_edited(&path, &new_factor, &(*closure_transaction_register), &globals)
                });
                renderer.set_property_editable(true);
                let column: TreeViewColumn =
                    create_tree_view_text_column(&renderer, "Value", STORE_VALUE);
        ...
        view.connect_key_press_event(move |_accounts_view: &TreeView, event_key: &EventKey| {
            ...
            if masked_state == ModifierType::CONTROL_MASK.bits() {
                match event_key.get_keyval() {
                    key::n => {
                        new_split(&transaction_register_key_press_event, &globals_key_press_event);
                        Inhibit(true)
                    }
                    key::d => {
                        duplicate_split(
                    ...
                    key::s => {
                        display_split_account_register(
                            &transaction_register_key_press_event,
                            &globals_key_press_event,
                        );
                        Inhibit(true)
        ...
        window.connect_delete_event(move |_, _| {
            ...
            let balance: f64 = stmt
                .query_row(params![(&*transaction_register_delete_event.guid)], get_result!(f64))
                .unwrap();
            if balance.abs() > EPSILON {
                display_message_dialog("Transaction final balance must be zero", &globals);
                Inhibit(true)
            } else if globals_delete_event
                .transaction_registers
                .borrow_mut()
                ...
                .is_some()
            {
                ...
                panic!(
                    "Transaction register deleted, but not found in \
