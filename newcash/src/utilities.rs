...
use glib::types::Type;
use gtk::prelude::{ComboBoxExtManual, GtkListStoreExtManual, IsA};
use gtk::{
    Align, ButtonsType, CellLayoutExt, CellRendererText, CellRendererToggle, ComboBoxText,
    ComboBoxTextExt, ContainerExt, Dialog, DialogExt, DialogFlags, Entry, EntryBuffer, EntryExt,
    Grid, GridExt, Label, ListStore, MessageDialog, MessageType, ResponseType, TreeIter, TreeModel,
...
pub fn maybe_date(date: &str, globals: &Globals) -> bool {
    prepare_statement!("select julianday(?1)", globals)
        .query_row(params![date.to_string()], |row| -> Result<f64, rusqlite::Error> { row.get(0) })
        .is_ok()
}

pub fn date_edited(
    guid: &str, increment_stmt: &mut Statement, beginning_of_month_stmt: &mut Statement,
    end_of_month_stmt: &mut Statement, today_stmt: &mut Statement, user_entry_stmt: &mut Statement,
    new_date: &str, globals: &Globals,
) {
    ...
    if let Some(increment) = maybe_date_increment(trimmed_new_date) {
        ...
        user_entry_stmt.execute(params![(trimmed_new_date.to_string()), guid]).unwrap();
    } else {
        display_message_dialog(DATE_ERROR_MESSAGE, globals);
    }
}

pub fn get_string_column_via_path<T: TreeModelExt>(
    model: &T, path: &TreePath, column: i32,
) -> String {
    if let Some(iter) = model.get_iter(path) {
        model.get_value(&iter, column).get().unwrap().unwrap()
    } else {
        panic!("get_string_column_via_path: gtk_tree_model_get_iter failed");
    }
}

pub fn get_boolean_column_via_path<T: TreeModelExt>(
    model: &T, path: &TreePath, column: i32,
) -> bool {
    if let Some(iter) = model.get_iter(path) {
        model.get_value(&iter, column).get().unwrap().unwrap()
...
pub fn select_row_by_guid(
    view: &TreeView, guid: &str, store_guid_column: i32, focus_column: &TreeViewColumn,
) {
    ...
    if let Some(iter) = get_iter_to_last_row(&model) {
        loop {
            let current_guid: String =
                model.get_value(&iter, store_guid_column).get().unwrap().unwrap();
            if current_guid == guid {
                ...
                if let Some(path) = model.get_path(&iter) {
                    ...
                    panic!(
                        "select_row_by_guid: after searching by guid and finding a match, \
                         get_path failed"
                    );
                }
...
pub fn column_index_to_column(view: &TreeView, column_index: i32) -> TreeViewColumn {
    if let Some(column) = view.get_column(column_index) {
        ...
        panic!(
            "column_index_to_column: get_column returned None. Column index was {}",
            column_index
        );
    }
}
...
pub fn update_string_column_via_path(
    store: &ListStore, path: &TreePath, new_string: &str, column: i32,
) {
...
pub fn update_boolean_column_via_path(
    store: &ListStore, path: &TreePath, new_boolean: bool, column: i32,
) {
    if let Some(iter) = store.get_iter(path) {
...
pub fn create_tree_view_text_column(
    renderer: &CellRendererText, title: &str, store_column_index: i32,
) -> TreeViewColumn {
...
pub fn create_tree_view_toggle_column(
    renderer: &CellRendererToggle, title: &str, store_column_index: i32,
) -> TreeViewColumn {
    let column = TreeViewColumn::new();
    column.pack_start(renderer, true);
...
pub fn find_search(
    find_command: &FindCommand, find_parameters: &RefCell<FindParameters>,
    ...
    if let Some(ref regex) = find_parameters_borrow.regex {
        ...
        let column_index =
            find_parameters_borrow.column_index.expect("Find: non-existent column index");
        loop {
            let column_value: String = match find_parameters_borrow
                .column_type
                .expect("Find: in search, column_type is missing")
            {
                Type::String => model.get_value(&iter, column_index).get().unwrap().unwrap(),
                Type::Bool => {
                    let temp: bool = model.get_value(&iter, column_index).get().unwrap().unwrap();
                    format!("{}", temp)
                }
            ...
            if regex.is_match(&column_value) {
                ...
                let column: TreeViewColumn = view
                    .get_column(find_parameters_borrow.default_view_column.try_into().unwrap())
...
fn find_dialog(
    find_parameters: &RefCell<FindParameters>, register_core: &RegisterCore, globals: &Globals,
) -> bool {
    ...
    if dialog.run() == ResponseType::Ok {
        ...
        if let Ok(regex) = Regex::new(
            format!(
                "(?i){}",
                regular_expression.get_text().expect(
                    "Find: failed to obtain \
                     regex from dialog"
                )
            )
            .as_str(),
            ...
            panic!(
                "Find: unable to locate specified column. This is a bug in Newcash. Please \
                 report to Don Allen."
            );
        } else {
            display_message_dialog(
                "The find expression was not a valid regular expression.",
                globals,
            );
            dialog.destroy();
            false
