...
impl<'a> Globals<'a> {
    fn fix_missing_commodity(&mut self, account: &Account) {
        let new_commodity_guid = self.new_guid
                                     .query_row(params![], get_result!(string))
                                     .unwrap();
        // Create new commodity
        self.insert_new_commodity
            .execute(params![new_commodity_guid, account.name])
            .unwrap();
        // And link the account to the new commodity
        self.link_to_commodity
            .execute(params![new_commodity_guid, account.guid])
            .unwrap();
    }
    fn check_and_repair_commodity_link(&mut self, account: &Account) {
        if account.commodity_guid == "" {
            let possible_commodity_guid =
                self.get_possible_commodity_guid
...
A commodity with the same name as the account does exist. The account will be linked to it.",
            ...
            match self.verify_commodity_guid
...
but the commodity name is not the same as the account name.  Is this intentional?",
    ...
    fn walk_account_tree(&mut self, account: &Account, ancestor_flags: i32) {
        ...
        commodity. Removing the association by setting the account's commodity link to NULL.",
                        ...
                        self.nullify_commodity_guid
                            .execute(params![account.guid])
                            .unwrap();
                    }
                    // Make sure the quantity is zero. Should not be otherwise for a non-marketable asset.
                    if self.check_quantities
                           .query_row(params![account.guid], get_result!(i32))
                           .unwrap()
                       > 0
                    {
                        println!("{} is a non-marketable asset account but has splits with \
    ...
    should not be possible and is indicative of a Newcash bug. Please report this to Don Allen.",
                ...
                if self.check_quantities
                   ...
                   > 0
                {
                    println!("{} is not an asset account but has splits with non-zero \
...
Usage: newcashverifier pathToDatabase",
    ...
    {
        let mut check_table_stmt = db.prepare("select count(*) from sqlite_master where \
                                               tbl_name=?1")
                                     .unwrap();
        let mut check_table = |table: &'static str, create_sql: &'static str| {
            if check_table_stmt.query_row(params![table], get_result!(i32))
                               .unwrap()
               == 0
            {
        ...
        let mut create_root_child_stmt = db.prepare(concat!(
            ...
            ", ?1,
                          (select root_account_guid from book), '', '', ?2)"
                                           ...
                                           .unwrap();
        let mut update_child_flags_stmt = db.prepare("update accounts set flags = ?1 where guid \
                                                      = ?2")
                                            .unwrap();
        let mut process_root_child =
            |account_name: &str, correct_flags: i32, bits_to_ignore: i32| {
                let mut get_root_child_iter =
                    get_root_child_stmt.query_map(params![account_name], |row| {
                                           Ok(AccountInfo { guid: row.get(0).unwrap(),
                ...
                match get_root_child_iter.next() {
                    Some(root_child) => {
                        ...
                        if (r.flags & mask) != (correct_flags & mask) {
                            ...
                            println!("The current flags of Root:{} are not correct. Current \
                                      value: 0x{:x}. Should be 0x{:x}. This will be fixed.",
                                     account_name, r.flags, correction);
                            update_child_flags_stmt.execute(params![correction, r.guid])
                                                   .unwrap();
                        }
...

    {
        ...
        let mut globals =
            Globals { new_guid: db.prepare(concat!("select ", constants!(NEW_UUID)))
                                  .unwrap(),
    ...
    let no_transaction_count: i32 =
        db.query_row("select count(*) from splits s left outer join transactions t on s.tx_guid \
                      = t.guid
    ...
    if no_transaction_count > 0 {
        println!("There were {} splits pointing to non-existent transactions. They will be \
                  deleted.",
                 no_transaction_count);
        db.execute("delete from splits where guid in
                        (select s.guid
    ...
    // List transactions that have splits that point to non-existent accounts
    {
        ...
        let mut stmt = db.prepare(
            ...
            ",
                         ...
                         .unwrap();
        let no_account_iter = stmt.query_map(params![], get_result!(string_string))
                                  .unwrap();
        for result in no_account_iter {
            let (date, description) = result.unwrap();
            if first_p {
                println!("The following transactions have splits pointing to a non-existent \
    ...
    let no_split_count: i32 =
        db.query_row("select count(*) from transactions
                    where guid not in (select tx_guid from splits)",
                     params![],
                     get_result!(i32))
          .unwrap();
    if no_split_count > 0 {
        println!("There were {} transactions that have no splits. They will be deleted.",
                 no_split_count);
        db.execute("delete from transactions where guid in
    ...
    // Check all transactions to be sure they are balanced
    {
        ...
        let balance_check_iter =
            balance_check_stmt.query_map(params![], |row| {
                                  Ok(NotBalanced { post_date: row.get(0).unwrap(),
        ...
        for transaction in balance_check_iter {
            ...
            let split_accounts_iter = split_accounts_stmt.query_map(params![t.transaction_guid],
                                                                    get_result!(string))
                                                         .unwrap();
    ...
    // Check for duplicate symbols
    {
        ...
        let duplicate_symbol_check_iter = duplicate_symbol_check_stmt.query_map(params![], |row| {
    ...
    {
        let mut orphans_stmt = db.prepare("select name, guid
                                            from accounts
                                            where parent_guid is null and guid not in (select \
                                           root_account_guid from book)")
                                 .unwrap();
        let orphans_iter = orphans_stmt.query_map(params![], get_result!(string_string))
                                       .unwrap();
        for wrapped_orphan in orphans_iter {
            let (name, guid) = wrapped_orphan.unwrap();
            println!("Orphaned account {} found. Temporarily making Root its parent. Please use \
            ...
            let mut stmt = db.prepare_cached("update accounts set name = name||'.'||guid, \
